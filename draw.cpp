...
void Draw::drawEsp(int x, int y, float distance, HBRUSH hBrush, COLORREF colorBol, int widths, int heights) {
	int w = widths;
	int h = heights;
	float newWidth = (0.37f * widths) + w;  // Use float and 'f' to indicate a float literal
	float newHeight = (3.00f * heights) + h;  // Adjusted from 300 to 3.00 assuming it's a scaling factor

	int width = static_cast<int>(newWidth / distance);
	int height = static_cast<int>(newHeight / distance);
